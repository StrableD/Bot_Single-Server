...
async def test_gm_approval_loop(setup_test_db):
    ...
    async with setup_test_db() as session:
        ...
        result = await session.execute(
            select(ActionRequest).where(ActionRequest.status == "pending")
        )
        ...
        result = await session.execute(
            select(ActionRequest).where(ActionRequest.id == pending[0].id)
        )
...
async def test_is_gamemaster(setup_test_db, monkeypatch):
    ...
    # Normal user
    try:
        assert not await predicate(ctx)
    except Exception:
        pass  # Often raises CheckFailure

    # Server Owner
    ctx.author.id = 111
    assert await predicate(ctx)
