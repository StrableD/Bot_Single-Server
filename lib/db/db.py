...
from lib.db.models import Base, Channel, League, Player, Role
...
async def getChannelID(bot_name: str) -> int:
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(Channel).where(Channel.name_bot == bot_name)
        )
...
async def getElo(player_id: int):
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(Player).where(Player.PlayerId == player_id)
        )
...
async def setPlayerElo(player_id: int, new_elo: int):
    async with AsyncSessionLocal() as session:
        await session.execute(
            update(Player).where(Player.PlayerId == player_id).values(Elo=new_elo)
        )
...
async def resetSeason():
    async with AsyncSessionLocal() as session:
        await session.execute(
            update(Player).values(PlayedGamesSeason=0, WonGamesSeason=0, Elo=1300)
        )
        await session.commit()
    ...
    async with AsyncSessionLocal() as session:
        ...
        for member in members:
            if member.id in player_ids:
                await session.execute(
                    update(Player)
                    .where(Player.PlayerId == member.id)
                    .values(PlayerName=member.display_name)
                )
            else:
