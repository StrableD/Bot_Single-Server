...
class GamemasterDashboard(View):
    ...
    @discord.ui.button(label="Advance Phase", style=discord.ButtonStyle.primary, custom_id="gm_dash_next")
    async def next_phase_btn(self, interaction: discord.Interaction, button: Button):
        ...
        if next_cog:
            ...
            await interaction.response.send_message("Advancing phase...", ephemeral=True)
        else:
            await interaction.response.send_message("Next cog not loaded.", ephemeral=True)

    @discord.ui.button(label="Kill Player", style=discord.ButtonStyle.danger, custom_id="gm_dash_kill")
    async def kill_btn(self, interaction: discord.Interaction, button: Button):
        await interaction.response.send_message("Use /dead command for now. Dropdown coming soon.", ephemeral=True)

    @discord.ui.button(label="Revive Player", style=discord.ButtonStyle.success, custom_id="gm_dash_revive")
    async def revive_btn(self, interaction: discord.Interaction, button: Button):
        await interaction.response.send_message("Revive player logic here.", ephemeral=True)
...
class Dashboard(Cog):
    ...
    async def spawn_dashboard(self, ctx: Context):
        ...
        if "bot" not in channel_name and "gm" not in channel_name and "gamemaster" not in channel_name:
            await ctx.send("⚠️ **Warning**: You are spawning the dashboard outside the designated Gamemaster/Bot channel!", ephemeral=True)
