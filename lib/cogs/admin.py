...
class SetupWizard(discord.ui.View):
    ...
    async def start(self):
        ...
        btn_manual = discord.ui.Button(
            label="Manual", style=discord.ButtonStyle.blurple
        )
        btn_cancel = discord.ui.Button(
            label="Cancel Setup", style=discord.ButtonStyle.danger, row=2
        )
        ...
        async def auto_cb(i: discord.Interaction):
            ...
            await i.response.edit_message(
                content="Starting Automatic Setup...", embed=None, view=self
            )
        ...
        async def manual_cb(i: discord.Interaction):
            ...
            self.clear_items()
            await i.response.edit_message(
                content="Starting Manual Setup...", embed=None, view=self
            )
            await self.next_step(i)
    ...
    async def next_step(self, interaction: discord.Interaction):
        if self.current_idx >= len(self.pending_roles):
            await interaction.edit_original_response(
                content="✅ Setup Complete! All roles have been mapped.",
                embed=None,
                view=None,
            )
            return
    ...
    async def save_role_to_db(
        self, bot_name: str, guild_name: str, discord_role_id: int
    ):
        async with AsyncSessionLocal() as session:
            bot_name_clean = bot_name.lower().replace("-", "").replace(" ", "")
            result = await session.execute(
                select(DBRole).where(DBRole.name_bot == bot_name_clean)
            )
            db_role = result.scalar_one_or_none()
            if not db_role:
                db_role = DBRole(
                    name_bot=bot_name_clean,
                    id=discord_role_id,
                    name_guild=guild_name,
                    team="unknown",
                )
                session.add(db_role)
            else:
    ...
    async def handle_auto_step(self, interaction: discord.Interaction, role_name: str):
        ...
        if guessed_role:
            embed = discord.Embed(
                title=f"Role Setup: {role_name}",
                description=f"Found unmapped Discord role **{guessed_role.name}** (ID: `{guessed_role.id}`).\nMap this to game role **{role_name}**?",
                color=discord.Color.gold(),
            )

            btn_yes = discord.ui.Button(label="Yes", style=discord.ButtonStyle.green)
            btn_no = discord.ui.Button(
                label="No, ask to create", style=discord.ButtonStyle.danger
            )

            async def yes_cb(i: discord.Interaction):
                await self.save_role_to_db(
                    role_name, guessed_role.name, guessed_role.id
                )
                self.current_idx += 1
    ...
    async def prompt_create(
        self, interaction: discord.Interaction, role_name: str, target_name: str
    ):
        self.clear_items()
        embed = discord.Embed(
            title=f"Role Setup: {role_name}",
            description=f"I couldn't find an unmapped Discord role named **{target_name}**.\nShould I create it for you?",
            color=discord.Color.red(),
        )

        btn_create = discord.ui.Button(
            label=f"Create as '{target_name}'", style=discord.ButtonStyle.green
        )
        ...
        async def create_cb(i: discord.Interaction):
            ...
            try:
                ...
                await i.followup.send(
                    "I lack permissions to create roles! Skipping...", ephemeral=True
                )
            self.current_idx += 1
            await self.next_step(i)
    ...
    async def handle_manual_step(
        ...
        unmapped_roles = [
            ...
            if r.id not in mapped_ids and r.name != "@everyone"
        ]
        ...
        btn_cancel = discord.ui.Button(
            label="Cancel Setup", style=discord.ButtonStyle.danger, row=1
        )
        ...
        if not unmapped_roles:
            embed = discord.Embed(
                title=f"Role Setup: {role_name}",
                description="There are no more unmapped roles available in the server!",
                color=discord.Color.red(),
            )
            self.add_item(btn_skip)
            self.add_item(btn_cancel)
        ...
        embed = discord.Embed(
            title=f"Role Setup: {role_name}",
            description=f"Please select the Discord role that corresponds to the game role **{role_name}**.\nOnly unmapped roles are shown below.",
            color=discord.Color.purple(),
        )

        options = [
            discord.SelectOption(
                label=r.name[:100], description=f"ID: {r.id}", value=str(r.id)
            )
            for r in unmapped_roles
        ]
        select_menu = discord.ui.Select(
            placeholder="Choose a role...", min_values=1, max_values=1, options=options
        )
...
class Admin(Cog):
    ...
    @admin_group.command(
        name="setup", description="Run the server setup wizard to initialize the bot."
    )
    async def setup_wizard(self, interaction: discord.Interaction):
        if (
            ...
            await interaction.response.send_message(
                "Only the Server Owner or an Administrator can run the setup wizard.",
                ephemeral=True,
            )
            return
